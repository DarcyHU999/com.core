...
class API:
    def __init__(self, id, key, botoType, service, region):
        ...
        if self.service == 'ec2':
            self.raw_data = client.describe_instances()
            self.type_id_name = "InstanceId"
        elif self.service == 'subnet':
            self.raw_data = client.describe_subnets()
            self.type_id_name = "SubnetId"
        elif self.service == 'vpc':
            self.raw_data = client.describe_vpcs()
            self.type_id_name = "VpcId"
        elif self.service == 'security group':
            self.raw_data = client.describe_security_groups()
            self.type_id_name = "GroupId"
        elif self.service == 'dynamodb':
            self.raw_data = client.describe_table(TableName='posts')
        elif self.service == 's3':
            self.raw_data = client.list_buckets()
        else:
        ...
        for group in self.raw_data[key]:
            ...
            if 'Tags' in tags_loc:
                ...
                if self.service != 'ec2':
                    ...
                    self.num_of_tags_in_groups[group['Instances'][0]['InstanceId']] = len(group['Instances'][0]['Tags'])
